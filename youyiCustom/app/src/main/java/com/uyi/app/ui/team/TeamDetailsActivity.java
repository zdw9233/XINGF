...
public class TeamDetailsActivity extends BaseActivity implements android.widget.AdapterView.OnItemClickListener {
	...
	@ViewInject(R.id.team_headerView) private HeaderView team_headerView;
	@ViewInject(R.id.team_group_logo) private ImageView team_group_logo;
	@ViewInject(R.id.team_group_info) private TextView team_group_info;
	@ViewInject(R.id.team_group_name) private TextView team_group_name;
	@ViewInject(R.id.team_zl_group_logo) private       RoundedImageView team_zl_group_logo;
	@ViewInject(R.id.team_zl_name) private TextView team_zl_name;
	@ViewInject(R.id.team_zl_phone) private TextView team_zl_phone;
	@ViewInject(R.id.team_zl_group_info) private TextView team_zl_group_info;
	@ViewInject(R.id.tezm_money) private TextView tezm_money;
	@ViewInject(R.id.team_zixunshu) private TextView team_zixunshu;
	@ViewInject(R.id.team_zonzixunshu) private TextView team_zonzixunshu;
	@ViewInject(R.id.team_group_docter) private ListView team_group_docter;
	@ViewInject(R.id.team_goup_jiaru) private Button team_goup_jiaru;
	@ViewInject(R.id.team_details_rating) private RatingBar team_details_rating;
	int id;//健康团队id
	int isMy;//是否已加入
	String detail;//团队详情
	int doctorId;
...
//					System.out.println(assistant.getString("name")+"name");
//					System.out.println(assistant.getString("phoneNumber")+"name2");
//					System.out.println(assistant.getString("info")+"name3");
					ImageCacheManager.loadImage(assistant.get("icon").toString(), ImageCacheManager.getImageListener(team_zl_group_logo, null, null));
					
			
//					if(type == 2){
//						team_goup_jiaru.setVisibility(View.VISIBLE);
//				
//					}
...
//						myGroup.put("type",1);//判断是否加入的团队
	...
	protected void onBuildVersionGT_KITKAT(SystemBarConfig systemBarConfig) {
		team_headerView.setKitkat(systemBarConfig);
					        ...
					        RoundedImageView   icon = (RoundedImageView)layout.findViewById(R.id.item_zhuanjia_icon);
					         ...
					         name.setText(mydoctor.get("realName").toString());
				      		...
				      		recyclerView.setAdapter(docterDtailsAdapter);				     
				            	...
				            	RequestManager.getObject(String.format(Constens.HEALTH_GROUPS_DOCTER_DETAILS_PINJIA,mydoctor.get("id"),pageNo,pageSize/2),activity, new Listener<JSONObject>() {
									@Override
									public void onResponse(JSONObject data) {
										try {
											datapingjia.clear();
