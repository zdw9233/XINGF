 ...
 *
 */
public class ThreeTopListAdapter extends BaseRecyclerAdapter<Map<String,Object>> {

	public Context context;
	private ArrayList<Integer> showDetailPositions;
	public ThreeTopListAdapter(Context context) {
	...
	public void onBind(final ViewHolder viewHolder, final int RealPosition, final Map<String, Object> data) {
		if(viewHolder instanceof Holder){
			...
			if(data.get("status").toString().equals("PASS")){
				hodler.zhuantai.setText("通过");
			}else if(data.get("status").toString().equals("NOPASS")){
				hodler.zhuantai.setText("未通过");
			}else if(data.get("status").toString().equals("UNREVIEWED")){
