...
public class UpdateCalendarActivity extends BaseActivity{
	...
	protected void onInitLayoutAfter() {
		...
		fangansj.setText(date);
		fanganxz.setText(content);
				...
				try {
					...
					if(json.length()>0){
						for (int i = 0; i < json.length(); i++) {
							...
							CheckBox box = new CheckBox(UpdateCalendarActivity.this);
						...
						for (int z = 0; z < checkboxs.size(); z++) {
							if(checkboxs.get(z).getText().toString().equals(content)){
				...
				conform.setOnMessageClick(new MessageConform.OnMessageClick() {
					...
					public void onClick(MessageConform.Result result, Object object) {
						if (result == MessageConform.Result.OK) {
							...
							RequestManager.postObject(Constens.GET_SERVER_FANGAN_UP, activity, params, new Response.Listener<JSONObject>() {
										public void onResponse(JSONObject arg0) {
											T.showShort(UpdateCalendarActivity.this,"修改成功！");
											setResult(RESULT_OK);
											finish();
										}
									},
