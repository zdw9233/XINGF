 ...
 *
 */
public class FragmentHealthDatabase extends BaseFragment implements OnTabChanage, OnItemClickListener<Map<String,Object>>, Pager, OnRefreshListener {
...
//	@ViewInject(R.id.new_health_database_layout_start) private LinearLayout new_health_database_layout_start;
	...
	public FragmentHealthDatabase(HealthManagerMain healthManagerMain) {
...
//		headerView.showLeftHeader(true,UserInfoManager.getLoginUserInfo(context).icon).showTab(true).showRight(true) .setOnTabChanage(this);
		...
		healthDatabaseAdapter = new HealthDatabaseAdapter(getView().getContext());
...
//		}
	...
	public void onItemClick(int position, Map<String,Object> data) {
		...
		Intent intent  = new Intent(context, HealthDatabaseDetails.class);
				...
				if(selectedDate == 1){
					try {
						...
						if(stime > System.currentTimeMillis()){
							T.showShort(context, "开始日期不能大于当前日期");
							return;
						}
