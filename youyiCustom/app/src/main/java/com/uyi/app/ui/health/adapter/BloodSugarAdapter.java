 ...
 *
 */
public class BloodSugarAdapter extends BaseRecyclerAdapter<Map<String,Object>>{
	...
	public ViewHolder onCreate(ViewGroup parent, int viewType) {
		View view = LayoutInflater.from(context).inflate(R.layout.item_blood_sugar, parent, false);
	...
	public void onBind(ViewHolder viewHolder, int RealPosition, Map<String, Object> data) {
		if(viewHolder instanceof Holder){
			...
			if(!data.get("fastBloodSugar").equals("NULL")){
				hodler.title2.setText(data.get("fastBloodSugar").toString());
				if(!data.get("fastBloodSugarWarning").equals("NULL")){
					//获取资源图片
...
//					hodler.title2.setCompoundDrawablesWithIntrinsicBounds(R.drawable.success_icon, 0, 0, 0);
			...
			if(!data.get("postPrandilaSugar").equals("NULL")){
				hodler.title3.setText(data.get("postPrandilaSugar").toString());
				if(!data.get("postPrandilaSugarWarning").equals("NULL")){
					//获取资源图片
					Drawable leftDrawable = context.getResources().getDrawable(R.drawable.warning);
					//设置图片的尺寸，奇数位置后减前得到宽度，偶数位置后减前得到高度。
					leftDrawable.setBounds(0, 0, 20, 20);
...
//					hodler.title3.setCompoundDrawablesWithIntrinsicBounds(R.drawable.success_icon, 0, 0, 0);
			...
			if(!data.get("randomBloodSugar").equals("NULL")){
				hodler.title4.setText(data.get("randomBloodSugar").toString());
				if(!data.get("randomBloodSugarWarning").equals("NULL")){
					//获取资源图片
					Drawable leftDrawable = context.getResources().getDrawable(R.drawable.warning);
					//设置图片的尺寸，奇数位置后减前得到宽度，偶数位置后减前得到高度。
					leftDrawable.setBounds(0, 0, 20, 20);
