 ...
 * @author user
 */
public class BloodPressureAdapter2_1 extends BaseRecyclerAdapter<Map<String, Object>> {
    public Context context;

    public BloodPressureAdapter2_1(Context context) {
        this.context = context;
    }

    @Override
    public ViewHolder onCreate(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_blood_pressure2_1, parent, false);
    ...
    public void onBind(ViewHolder viewHolder, int RealPosition, Map<String, Object> data) {
        if (viewHolder instanceof Holder) {
            ...
            if (data.get("morningSystolicPressure") != null) {
                if (!data.get("morningSystolicPressure").toString().equals("0")) {
                    ...
                    if (data.get("mspState") != null) {
                        if (data.get("mspState").toString().equals("0")) {
                            hodler.title2.setCompoundDrawables(null, null, null, null);
                            hodler.title2.setTextColor(ContextCompat.getColor(context, R.color.tet_type_6));
                        } else if (data.get("mspState").toString().equals("1")) {
                            //获取资源图片
            ...
            if (data.get("morningDiastolicPressure") != null) {
                if (!data.get("morningDiastolicPressure").toString().equals("0")) {
                    ...
                    if (data.get("mdpState") != null) {
                        if (data.get("mdpState").toString().equals("0")) {
                            hodler.title3.setCompoundDrawables(null, null, null, null);
                            hodler.title3.setTextColor(ContextCompat.getColor(context, R.color.tet_type_6));
                        } else if (data.get("mdpState").toString().equals("1")) {
                            //获取资源图片
                            Drawable leftDrawable = context.getResources().getDrawable(R.drawable.high2x);
                            //设置图片的尺寸，奇数位置后减前得到宽度，偶数位置后减前得到高度。
                            leftDrawable.setBounds(0, 0, 20, 20);
            ...
            if (data.get("pulseRate") != null) {
                if (!data.get("morningDiastolicPressure").toString().equals("0")) {
                    ...
                    if (data.get("prState") != null) {
                        if (data.get("prState").toString().equals("0")) {
                            hodler.title4.setCompoundDrawables(null, null, null, null);
                            hodler.title4.setTextColor(ContextCompat.getColor(context, R.color.tet_type_6));
                        } else if (data.get("prState").toString().equals("1")) {
                            //获取资源图片
                            Drawable leftDrawable = context.getResources().getDrawable(R.drawable.high2x);
                            //设置图片的尺寸，奇数位置后减前得到宽度，偶数位置后减前得到高度。
                            leftDrawable.setBounds(0, 0, 20, 20);
