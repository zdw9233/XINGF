...
//        initData();
	...
	private void getData() {
		...
		RequestManager.getArray(String.format(Constens.GET_SERVER_XIANMU, customerId), activity,new Response.Listener<JSONArray>() {
				...
				try {
					...
					if(json.length()>0){
						for (int i = 0; i < json.length(); i++) {
							...
							CheckBox box = new CheckBox(AddCalendarActivity.this);
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.new_report:
				...
				for (int j = 0; j <checkboxs.size(); j++) {
					if (checkboxs.get(j).isChecked()) {
						id = datas.get(j).get("id").toString();
						num ++;
					}
				}
				if(num == 0){
					T.showShort(AddCalendarActivity.this,"至少选择一个事件！");
				}else if(num > 1){
					T.showShort(AddCalendarActivity.this,"每次选择一个事件！");
				}else{
...
//                    params.put("id", NewFollowUpDetailsActivity.id);
						...
						params.put("date",fangansj.getText().toString());
						params.put("customerId", customerId);
					} catch (JSONException e) {
						e.printStackTrace();
					}
					System.out.println(params.toString());
					RequestManager.postObject(Constens.GET_SERVER_FANGAN_BUCHON,activity, params, new Response.Listener<JSONObject>() {
						public void onResponse(JSONObject data) {
							System.out.println(data.toString());
							T.showShort(AddCalendarActivity.this,"补充成功！");
							setResult(RESULT_OK);
							finish();
//                        try {
//
